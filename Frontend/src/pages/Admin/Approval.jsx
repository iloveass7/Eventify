...
const Approval = () => {
  ...
  const fetchPastEvents = async () => {
    ...
    try {
      ...
      if (data.success) {
        // fetch fully-populated details per event
  ...
  const toggleAttendance = async (eventId, userId) => {
    ...
    // optimistic update first (preserve existing populated attendees)
    setPastEvents((prev) =>
      prev.map((e) => {
        if (e._id !== eventId) return e;
        const attendedBy = Array.isArray(e.attendedBy) ? e.attendedBy : [];
        const isAttending = attendedBy.includes(userId);
        const nextAttendedBy = isAttending
          ? attendedBy.filter((id) => id !== userId)
          : [...attendedBy, userId];
        return { ...e, attendedBy: nextAttendedBy };
      })
    );
    // keep the same dropdown open
    ...
    try {
      const response = await fetch(`${API_BASE}/api/event/${eventId}/attendance`, {
        ...
        body: JSON.stringify({
          ...
          action: (() => {
            ...
            const list = Array.isArray(current?.attendedBy) ? current.attendedBy : [];
            return list.includes(userId) ? "remove" : "add";
          })(),
        }),
      });
      ...
      // server may return an unpopulated event; refresh this one with populated details
      await refetchSingleEvent(eventId);
    } catch (err) {
      setError(err.message || "Failed to update attendance");
      // revert by refetching the event
      await refetchSingleEvent(eventId);
                  ...
                  {event.attendees && event.attendees.length > 0 ? (
                    <div className="max-h-80 overflow-y-auto space-y-2">
                      {event.attendees.map((attendee, index) => {
                        if (!attendee || !attendee._id) return null; // ignore unpopulated entries
                      ...
                      <div className="mb-3">
                        <svg className="w-12 h-12 mx-auto text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path
                            ...
                            strokeLinejoin="round"
                            strokeWidth={1}
                            d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197m13.5 0a4 4 0 11-8-4 4 0 018 4z"
                          />
                        </svg>
                      </div>
